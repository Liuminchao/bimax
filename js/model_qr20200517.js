...
async function onLoad() {
    ...
    if (model_id) {
        await data.getWINDDataLoader().openModelData(model_id,version,true);//打开对应模型id的模型数据
    ...
    for ( var i = 0; i <entity.length; i++){
        ...
        $.ajax({
            ...
            success: function(data){
                ...
                tab+="<tr><td ><span style='font-size: 15px;font-weight:bold;margin-right: 5px '>Block:</span><span>"+data.block+"</span></td> <td rowspan='6' align='right'><img src='"+data.filename+"' width='120' height='120'></td></tr>";
                tab+="<tr><td><span style='font-size: 15px;font-weight:bold;margin-right: 5px '>Part:</span><span>"+data.part+"</span></td></tr>";
                if(data.level){
                    tab+="<tr><td><span style='font-size: 15px;font-weight:bold;margin-right: 5px '>Level/Unit:</span><span>"+data.level+"</span></td></tr>";
                }else{
                    tab+="<tr><td><span style='font-size: 15px;font-weight:bold;margin-right: 5px '>Level/Unit:</span><span>"+floor+"</span></td></tr>";
                }
                if(data.unit_type){
                    tab+="<tr><td><span style='font-size: 15px;font-weight:bold;margin-right: 5px '>Unit Type:</span><span>"+data.unit_type+"</span></td></tr>";
                }else{
                    tab+="<tr><td><span style='font-size: 15px;font-weight:bold;margin-right: 5px '>Unit Type:</span><span>"+floor+"</span></td></tr>";
                }
                if(data.element_name){
                    tab+="<tr><td><span style='font-size: 15px;font-weight:bold;margin-right: 5px '>Element Name:</span><span>"+data.element_name+"</span></td></tr>";
                }else{
                    tab+="<tr><td><span style='font-size: 15px;font-weight:bold;margin-right: 5px '>Element Name:</span><span>"+name+"</span></td></tr>";
                }
                if(data.element_type){
                    tab+="<tr><td><span style='font-size: 15px;font-weight:bold;margin-right: 5px '>Element Type:</span><span>"+data.element_type+"</span></td></tr>";
                }else{
                    tab+="<tr><td><span style='font-size: 15px;font-weight:bold;margin-right: 5px '>Element Type:</span><span>"+type+"</span></td></tr>";
                }
                tab+="</table>";
...
async function selectqrchecklist() {
    ...
    $.ajax({
        ...
        type: "POST",
        data: {uuid: entity_id,model_id: model._id,template_id:template_id,program_id:program_id},
        dataType: "json",
        beforeSend: function () {
...
async function selectstatuslist() {
    ...
    console.log(status);
    view.getWINDViewControl().coloringBatchEntities(batch_id, 126, 127, 126, 111);
    if(status == 'true'){
        view.getWINDViewControl().coloringBatchEntities(batch_id, 126, 127, 126, 111);
    }
    ...
    $("#info").append(tab);
    $.ajax({
        data: {model_id: model._id,template_id:template_id,program_id:program_id},
        ...
        success: function (data) {
            ...
            $.each(data, function (name, value) {
                ...
                if(guid_list.length != 0){
                    ...
                    if(status == 'true'){
                        view.getWINDViewControl().coloringBatchEntities(batch_id, rgb.r, rgb.g, rgb.b, 111);
                    }
                }
...
async function colorComponent(stage_id,batch_id) {
    ...
    $.ajax({
        data: {model_id: model._id,stage_id:stage_id,program_id:program_id},
...
async function allcode() {
    ...
    addcloud();
    jQuery.ajax({
        data: {entity_str:entity_str, version:version, model_id:model_id,program_id:program_id},
        type: 'post',
        url: './index.php?r=rf/rf/getmodeldata',
        dataType: 'json',
        success: function (data, textStatus) {
//                $('#qr_table').append("</br>Loading...");
            ajaxReadData(program_id,version,model_id, data.rowcnt, 0);
        },
        error: function(XMLHttpRequest, textStatus, errorThrown){
...
function revertHomePosition() {
    // view.getWINDViewControl().showComponents();
...
async function setcomponent() {
    ...
    jQuery.ajax({
        ...
        success: function (data, textStatus) {
            ...
            modal.url = "index.php?r=task/model/setentity&model_id="+model_id+"&version="+version+"&program_id="+program_id;
            modal.modal();
        },
        error: function(XMLHttpRequest, textStatus, errorThrown){
            alert(XMLHttpRequest);
