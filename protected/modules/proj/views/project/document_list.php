...
//    exit;
    ...
    foreach ($rows as $i => $row) {
        ...
        $delete_link = "<a href='javascript:void(0)' onclick='itemDelete(\"{$row['doc_path']}\",\"{$row['doc_id']}\",\"{$row['doc_name']}\",\"{$row['program_id']}\")'><i class=\"fa fa-fw fa-times\"></i>" . Yii::t('electronic_contract', 'delete') . "</a>";
        $download_link = "<a href='javascript:void(0)' onclick='itemDownload(\"{$row['doc_path']}\")'><i class=\"fa fa-fw fa-download\"></i>" . Yii::t('electronic_contract', 'download') . "</a>";
        if($value == '0'){
...
//            "index.php?r=document/company/preview&doc_path="+path+"&doc_id="+id
            $t->echo_td("<a href='index.php?r=document/company/preview&doc_path={$row['doc_path']}' target='_blank'>" . $row['doc_name'] . "</a>");
        }else{
            $t->echo_td("<a href='javascript:void(0)' onclick='window.open(\"{$row['doc_path']}\",\"_blank\")'>" . $row['doc_name'] . "</a>");
        }
        if($row['doc_use'] == 0){
            $t->echo_td("<img id=\"{$row['doc_id']}\" class='no_selected' src='img/star.png' onclick='set(\"{$row['doc_id']}\")'>");
        }else{
            $t->echo_td("<img id=\"{$row['doc_id']}\" class='selected' src='img/star_select.png' onclick='set(\"{$row['doc_id']}\")'>");
        }
...
//        $t->echo_td('<a id="tags" class="editable editable-click" href="#" data-type="select2" data-pk="1" data-title="Enter tags" data-original-title="" title="" style="" data-url="index.php?r=document/platform/settags&doc_id='.$row['doc_id'].'">'.$label_name.'</a>');
        $t->echo_td('<a href="#" class="editable editable-click tags" data-type="select"  data-pk="1"  data-url="index.php?r=document/company/settags&doc_id='.$row['doc_id'].'" data-title="Select Tag" >'.$tag_list[$row['label_id']].'</a>');
