            ...
            <div class="card-body" style="background-color: #F2F9FA;">
                <div class="row">
                    ...
                    <div class="col-10" style="background-color: #F2F9FA;text-align: center">
                        ...
                        $item_list = RfRecordItem::dealListBystep($check_id,'1');
                        ...
                        <div class="row" style="margin-top: 8px;">
                            <label for="program_name" class="col-sm-4 offset-md-1 control-label padding-lr5" style="text-align: left">Outcome</label>
                            <div class="col-sm-3 padding-lr5">
                                <select id="deal_type" name="rf[deal_type]" class="form-control" check-type="required" >
                                    <?php
                                    $form_id = $rf_model->form_id;
                                    if($form_id == 'RF00003'){
                                        ?>
                                        <option value="3" style="color: #00CC00">In-Principal No Objection</option>
                                        <option value="4" style="color: #00FF00">Accepted with Comments</option>
                                        <option value="6" style="color: #CC6600">Rejected</option>
                                        <option value="7" style="color: #FFA500">Re-test / Revise & Resubmit</option>
                                    <?php }else{ ?>
                                        <option value="11" style="color: #00FF00">Approved</option>
                                        <option value="12" style="color: #CC6600">Not Approved</option>
                                    <?php } ?>
                                </select>
                            </div>
                        </div>

                        <?php if(array_key_exists('rvo',$item_data)){ ?>

                            <?php
                            if($item_data['rvo'] == '1'){
                                echo "<div class='row' style='margin-top: 8px;'>
                            <label for='program_name' class='col-sm-4 offset-md-1 control-label padding-lr5' style='text-align: left'>RVO</label>
                                <div class='col-sm-3 padding-lr5'  style='text-align: left'>
                                    <input type='radio' id='rvo' name='rf[rvo]'  value='1' checked  > Approve
                                    <input type='radio' id='not_rvo' name='rf[rvo]'  value='2' style='padding-left: 10px;' > Reject
                                    <input type='radio' id='not_rvo' name='rf[rvo]'  value='0' style='padding-left: 10px;' > NA
                                </div>
                          </div>";
                            } ?>
                        <?php } ?>
                    ...
                    $.each(data, function (name, value) {
                        if (name == 'data') {
                            if(ra_type == '.pdf' || ra_type == '.jpg' || ra_type == '.png' || ra_type == '.jpeg'){
                                var $tr = $("<div class='row col-12 ' style='margin-top: 8px;'><div class='col-sm-7 padding-lr5' style='text-align: left;padding-left: 0px;'><img src='img/attach.png'>"+video_src_file+"</div><div class='col-sm-2 padding-lr5' style='text-align: left'>"+"<a class='a_logo' onclick='previewdoc(\""+value.file1+"\")'  style='cursor:pointer;'  title='Preview'><i class='fa fa-fw fa-eye'></i></a>"+"<a class='a_logo' onclick='del_attachment(this)'   style='cursor:pointer;'  title='Delete'><i class='fa fa-fw fa-times'></i></a><input type='hidden' name='rf[attachment][]' value='"+value.file1+"' ></div></div>");
                            }else{
                                var $tr = $("<div class='row col-12 ' style='margin-top: 8px;'><div class='col-sm-7 padding-lr5' style='text-align: left;padding-left: 0px;'><img src='img/attach.png'>"+video_src_file+"</div> "+"<div class='col-sm-2 padding-lr5' style='text-align: left'><a class='a_logo' onclick='del_attachment(this)'  style='cursor:pointer;'  title='Delete'><i class='fa fa-fw fa-times'></i></a><input type='hidden' name='rf[attachment][]' value='"+value.file1+"' ></div></div>");
                            }
                            var $table = $("#attach");
                            $table.append($tr);
                        }
