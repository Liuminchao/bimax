...
class RfDetail extends CActiveRecord {
    ...
    public static function insertList($args){
        ...
        $sql = "insert into rf_record_detail (check_id,step,user_id,deal_type,remark,status,link_check_id,record_time,params) values (:check_id,:step,:user_id,:deal_type,:remark,:status,:link_check_id,:record_time,:params)";
...
//                $trans->rollBack();
    ...
    public static function queryGroupByRecord($check_id){
        $sql = "SELECT b.user_name,b.contractor_name FROM rf_record_detail a left join rf_group_user b on a.user_id=b.user_id WHERE a.check_id='".$check_id."' and a.deal_type not in ('8','13') and a.status != '1' order by a.step DESC LIMIT 1";
        $command = Yii::app()->db->createCommand($sql);
        $rows = $command->queryAll();
        $detail = '';
        if (count($rows) > 0) {
            foreach ($rows as $key => $row) {
                $detail.=$row['user_name'].'<br>'.$row['contractor_name'];
            }
        }
