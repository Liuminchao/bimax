...
//        $end_date = date('Y-m-d',strtotime($meeting->end_date));
        ...
        $apply_info_html .="<tr><td height=\"50px\" align=\"center\" style=\"border-width: 1px;border-color:gray gray gray gray\">&nbsp;{$apply_user[0]['user_name']}</td><td align=\"center\" style=\"border-width: 1px;border-color:gray gray gray gray\">&nbsp;{$roleList[$apply_role]}</td><td align=\"center\" style=\"border-width: 1px;border-color:gray gray gray gray\">&nbsp;{$apply_user[0]['work_no']}</td><td  align=\"center\" style=\"border-width: 1px;border-color:gray gray gray gray\">&nbsp;{$company_list[$contractor_id]}</td></tr>";
...
//        $content = '/opt/www-nginx/web/filebase/record/2018/04/sign/pic/sign1523249465052_1.jpg';
        ...
        if($module_type == 1) {
            ...
            $work_content_html .="<tr><td align=\"center\" height=\"50px\" style=\"border-width: 1px;border-color:gray gray gray gray\">{$meeting->type_name_en}<br>({$type_name})</td><td align=\"center\" style=\"border-width: 1px;border-color:gray gray gray gray\">Office</td></tr>";
...
//        $pdf->writeHTML($pic_html, true, false, true, false, '');
        ...
        if (!empty($progress_list)) {
            $pic_html = '<table height="840px" style="border-width: 1px;border-color:lightslategray lightslategray lightslategray lightslategray">';
            $i = 0;
            foreach ($progress_list as $key => $row) {
                if($row['pic'] != '') {
                    $pic = explode('|', $row['pic']);
                    foreach ($pic as $key => $content) {
                        if ($content != '' && $content != 'nil' && $content != '-1') {
                            if(file_exists($content)){
                                $i++;
                                $img= '<img src="'.$content.'" height="300" width="300" />';
                                if($i == 3){
                                    $pic_html.='<td>'.$img.'</td></tr>';
                                    $i = 0;
                                }else if ($i == 1){
                                    $pic_html.='<tr><td>'.$img.'</td>';
                                }else if($i == 2){
                                    $pic_html.='<td>'.$img.'</td>';
                                }
                            }
...
//                $info_yy += 15.5;
            ...
            if (!empty($members)){
                ...
                foreach ($members as $user_id => $r) {
                    $worker_html .= '<tr><td align="center" style="border-width: 1px;border-color:gray gray gray gray">&nbsp;' . $i . '</td><td align="center" style="border-width: 1px;border-color:gray gray gray gray">&nbsp;' . $r['work_no'] . '</td><td align="center" style="border-width: 1px;border-color:gray gray gray gray">&nbsp;'.$company_list[$r['contractor_id']].'</td><td align="center" style="border-width: 1px;border-color:gray gray gray gray">&nbsp;' . $r['worker_name'] . '</td><td align="center" style="border-width: 1px;border-color:gray gray gray gray">&nbsp;'.$roleList[$r['role_id']].'</td></tr>';
                    $i++;
                }
            }
...
//        var_dump($month);
        ...
        if($pro_model->main_conid != $args['contractor_id']){
            $sql = "select count(training_id) as cnt,program_id,type_id from train_apply_basic  where  program_id = '".$args['program_id']."' and record_time like '".$month."%' and add_conid = '".$args['contractor_id']."' and module_type='".$args['module_type']."'  GROUP BY type_id";
        }else{
            //总包项目
...
//        var_dump($month);
        ...
        //分包项目
        if($pro_model->main_conid != $args['contractor_id']){
            $sql = "select count(training_id) as cnt,add_conid from train_apply_basic  where  record_time like '%".$month."%' and program_id ='".$args['program_id']."' and add_conid = '".$args['contractor_id']."' and module_type='".$args['module_type']."'  GROUP BY add_conid";
        }else{
            //总包项目
