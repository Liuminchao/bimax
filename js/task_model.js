...
canvas.addEventListener('click', function(e){
    var entity = view.getWINDViewRoaming().getSelectEntities();
    let modellistUI = document.getElementById("modellist");
    let model = modeldata.get(modellistUI.options[modellistUI.selectedIndex].text);
    var template_id = $('#template_id').val();
    $('#model_id').val(model._id);
    $('#template').val(template_id);
    if (entity.length != 0){
        $('#uuid').val(entity[0]);
        itemQuery();
    }
...
async function openModelData() {
    ...
    batch_id = '199202'
    status = view.getWINDViewControl().setBatchEntities(batch_id,uuid_list);
    if(status == 'true'){
    ...
    $.ajax({
        ...
        url: "index.php?r=task/task/queryprogressmodel",
        type: "POST",
        dataType: "json",
        beforeSend: function () {
        },
        success: function (data) {
            ...
            $.each(data, function (name, value) {
                ...
                if(guid_list.length != 0){
                    batch_id = '334433';
                    status = view.getWINDViewControl().setBatchEntities(batch_id,guid_list);
...
async function getEntityParameter() {
    ...
    $('#version').val(version);
    // uuid = 'd03dfd27-e4be-435d-92b4-69e3306132e4-00324daf,d03dfd27-e4be-435d-92b4-69e3306132e4-00325be9';
    // arr = uuid.split(',');
    // batch_id = '334433';
    // status = view.getWINDViewControl().setBatchEntities(batch_id,arr);
    // alert(status);
    // if(status == 'true'){
    //     view.getWINDViewControl().coloringBatchEntities(batch_id, 255, 165, 0, 111);
    // }
    itemQuery();
}
...
function initViewUI() {
    ...
    //添加视图回调
    var tag = view.addWINDViewCallback('callback', callback);
}
