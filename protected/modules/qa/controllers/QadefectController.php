 ...
 * @author LiuMinChao
 */
class QadefectController extends AuthBaseController
{
    ...
    private function genDataGrid()
    {
        ...
        $t->url = 'index.php?r=qa/qadefect/grid';
...
//        $args['status'] = ApplyBasic::STATUS_FINISH;
        ...
        if($args['source'] == 'inspection'){
//            $this->saveInspectionUrl();
            $list = QaDefect::queryInspectionList($page, $this->pageSize, $args);
        }
        if($args['source'] == 'DFMA'){
//            $this->saveDfmaUrl();
            $list = QaDefect::queryDfmaList($page, $this->pageSize, $args);
        }
//        var_dump($list['rows']);
        $this->renderPartial('_list', array('t' => $t,'source' => $args['source'], 'program_id' => $args['program_id'], 'rows' => $list['rows'], 'cnt' => $list['total_num'], 'curpage' => $list['page_num']));
    }
    ...
    public function actionInspectionList()
    {
        ...
        $this->smallHeader = 'Issues';
        $this->render('list', array('program_id' => $program_id,'source'=>$source,'args' => $args));
    }

    public function actionDfmaList()
    {
        $program_id = $_REQUEST['program_id'];
        $source = $_REQUEST['source'];
        $args = array();
    ...
    public function actionChecklist() {
        ...
        $this->smallHeader = 'Issues';
        $this->render('checklist',array('check_id'=>$check_id,'source'=>$source,'args'=>$args));
    }

    /**
     * 查询
     */
    public function actionCheckGrid($check_id,$source)
    {
        $page = $_GET['page'] == '' ? 0 : $_GET['page']; //当前页码
        $_GET['page'] = $_GET['page'] + 1;
        $args = $_GET['q']; //查询条件
...
//        exit;
        ...
        $args['contractor_id'] = Yii::app()->user->contractor_id;
        $list = QaDefect::queryBySource($page, $this->pageSize, $args);
//        var_dump($list['rows']);
     ...
     * 保存查询链接
     */
    private function saveInspectionUrl() {

        $a = Yii::app()->session['list_url'];
        $a['qa/qadefect/inspectionlist'] = str_replace("r=qa/qadefect/grid", "r=qa/qadefect/inspectionlist", $_SERVER["QUERY_STRING"]);
     ...
     * 保存查询链接
     */
    private function saveDfmaUrl() {

        $a = Yii::app()->session['list_url'];
        $a['qa/qadefect/dfmalist'] = str_replace("r=qa/qadefect/grid", "r=qa/qadefect/dfmalist", $_SERVER["QUERY_STRING"]);
        Yii::app()->session['list_url'] = $a;
    }
    ...
    public function actionDownloadAttachment() {
        ...
        exec('php /opt/www-nginx/web/test/bimax/protected/yiic merge createimage --param1='.$check_id.' >/dev/null  &');
