...
//        $sql.= " limit $page,$pageSize";
    ...
    public static function CreateIndex($project_id,$block){
        $sql = "SELECT count(*) as cnt FROM pbu_info  WHERE project_id=:project_id and model_id = '0' and block=:block and pbu_id not like '%-%'";
        $command = Yii::app()->db->createCommand($sql);
...
//                        $task_record_cnt++;
    ...
    public static function deletePbu($tag) {
        ...
        foreach($model_list as $i => $id){
            ...
            //删除的构件 如果 block+level+unit下只有这一个构件
            if(count($rows)==1){
                $status = ProgramLocation::STATUS_DISABLE;
                $sub_sql = 'UPDATE bac_program_location_q SET status=:status WHERE project_id=:project_id and block =:block and level=:level and unit=:unit ';
                $command = Yii::app()->db->createCommand($sub_sql);
                $command->bindParam(":status", $status, PDO::PARAM_STR);
                $command->bindParam(":project_id", $project_id, PDO::PARAM_STR);
                $command->bindParam(":block", $block, PDO::PARAM_STR);
...
//        }
    ...
    public static function SetOriginal($id,$type,$replace){
        ...
        foreach ($list as $i => $j){
            ...
            $pbu_id = $model->pbu_id;
            $block_old = $model->block;
            $level_old = $model->level;
            $unit_old = $model->unit_nos;
            //查询block+level+unit下有几个构件  如果只有一个 那就直接执行更新location 如果有多个那就执行insert操作
            $sql = "SELECT * FROM pbu_info WHERE status=0 and project_id = :project_id and model_id=:model_id and pbu_id=:pbu_id and block = :block and level=:level and unit=:unit";//var_dump($sql);
            $command = Yii::app()->db->createCommand($sql);
            $command->bindParam(":project_id", $project_id, PDO::PARAM_STR);
            $command->bindParam(":model_id", $model_id, PDO::PARAM_STR);
            $command->bindParam(":pbu_id", $pbu_id, PDO::PARAM_STR);
            ...
            $command->bindParam(":unit", $unit_old, PDO::PARAM_STR);
            $rows = $command->queryAll();
...
//            if($block != $block_old || $level != $level_old || $unit_old != $unit_nos){
//                if(count($rows) == 1){
...
//                    $command->bindParam(":level_old", $level_old, PDO::PARAM_INT);
//                    $command->bindParam(":unit_old", $unit_old, PDO::PARAM_INT);
