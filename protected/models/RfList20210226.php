...
class RfList extends CActiveRecord {
    ...
    public static function statusCss($key = null) {
        $rs = array(
            ...
            self::STATUS_TIMEOUT => 'label-danger'
        );
        return $key === null ? $rs : $rs[$key];
    }
    ...
    public static function queryList($page, $pageSize, $args = array()) {
        ...
        setcookie('rf_status', $args['status']);
        if($status != ''){
...
//                        exec('php /opt/www-nginx/web/test/bimax/protected/yiic mail send --param1='.$check_id.'  >/dev/null  &');
                        foreach($group_user as $i => $to_user_id){
                            $r = MailType::sendMail($to_user_id,$group_user,$cc_user,$args);
                            $txt = '['.$record_time.']'.'  '.'RF Mail'.': '.json_encode($r);
                            self::write_log($txt);
                        }
                        if(count($cc_user)>0){
                            foreach($cc_user as $j => $cc_user_id){
                                $r = MailType::sendMail($cc_user_id,$group_user,$cc_user,$args);
                                $txt = '['.$record_time.']'.'  '.'RF Mail'.': '.json_encode($r);
                                self::write_log($txt);
                            }
                        }
...
//            $trans = Yii::app()->db->beginTransaction();
//            try{
//                $sql = "insert into rf_record (check_id,form_id,type,check_no,record_id,subject,discipline,submission,current_step,project_id,project_name,contractor_id,contractor_name,status,apply_user_id,valid_time,apply_time,link_check_id) values (:check_id,:form_id,:type,:check_no,:record_id,:subject,:discipline,:submission,:current_step,:project_id,:project_name,:contractor_id,:contractor_name,:status,:apply_user_id,:valid_time,:apply_time,:link_check_id)";
//                $command = Yii::app()->db->createCommand($sql);
//                $command->bindParam(":check_id", $check_id, PDO::PARAM_STR);
//                $command->bindParam(":form_id", $args['template_type'], PDO::PARAM_STR);
//                $command->bindParam(":type", $args['type_id'], PDO::PARAM_STR);
...
//                    }
//                    //User
//                    $r = RfUser::insertList($args,$to_user_id);
//                    $txt = '['.$record_time.']'.'  '.'RF User'.': '.json_encode($r);
//                    self::write_log($txt);
//                    if($r['status'] == '-1'){
//                        $trans->rollBack();
...
//
//                    if($status == '0'){
//                        $r = MailType::sendMail($to_user_id,$cc_user,$args);
//                        $txt = '['.$record_time.']'.'  '.'RF Mail'.': '.json_encode($r);
//                        self::write_log($txt);
//                        if($r['status'] == '-1'){
//                            $trans->rollBack();
    ...
    public static function replyList($args){
        ...
        try{
            ...
            if ($rs) {
                ...
                foreach($to_user as $i => $to_user_id) {
                    $r = MailType::sendMail($to_user_id, $to_user, $cc_user, $args);
                }

                if(count($cc_user)>0){
                    foreach($cc_user as $n => $cc_user_id){
                        $r = MailType::sendMail($cc_user_id,$to_user,$cc_user,$args);
                    }
                }
...
//                $zip->open($filename,ZipArchive::CREATE);//创建一个空的zip文件
    ...
    public static function downloaddefaultPDF($params,$app_id){
        ...
        $filepath = Yii::app()->params['upload_report_path'].'/pdf/'.$year.'/'.$month.'/'.$program_id.'/rf/'.$contractor_id.'/' . $check_no .'.pdf';
        RfList::updatepath($id,$filepath);

        //$full_dir = Yii::app()->params['upload_record_path'].'/'.$year.'/'.$month.'/ptw';
        $full_dir = Yii::app()->params['upload_report_path'].'/pdf/'.$year.'/'.$month.'/'.$program_id.'/rf/'.$contractor_id;
...
//        $pdf->Header($logo_pic);
        ...
        $to_user_model = Staff::model()->findByPk($to_user);
        $to_user_con = $to_user_model->contractor_id;
        $to_con_model = Contractor::model()->findByPk($to_user_con);
        $to_con_name = $con_model->contractor_name;
        $to_user_name = $to_user_model->user_name;
        $to_role_id = $to_user_model->role_id;
        $to_role_name = $role_list[$to_role_id];
...
//        $signature_html_2= '<img src="'.$deal_signature_2.'" height="30" width="30" />';
        ...
        $info_html.="<tr><td style=\"border-width: 1px;border-color:gray white white gray\" width=\"20%\">Replied by: </td><td style=\"border-width: 1px;border-color:gray gray white white\" width=\"80%\">{$to_con_name}</td></tr>";
        $info_html.="<tr><td style=\"border-width: 1px;border-color:white white white gray\" width=\"20%\"></td><td style=\"border-width: 1px;border-color:white white white white\" width=\"20%\">{$to_user_name}</td><td style=\"border-width: 1px;border-color:white gray gray white\" width=\"60%\" rowspan=\"2\">{$signature_html_2}</td></tr>";
        $info_html.="<tr><td style=\"border-width: 1px;border-color:white white gray gray\" width=\"20%\"></td><td style=\"border-width: 1px;border-color:white white gray white\" width=\"20%\">{$to_role_name}</td></tr>";
...
//        $valid_time = $rf_model->valid_time;
        ...
        $info_html.="<tr><td  colspan=\"2\" width='50%' style=\"border-width: 1px;border-color:gray gray gray gray\">Prepared By:  {$add_user_name}</td><td  colspan=\"2\" width='50%' style=\"border-width: 1px;border-color:gray gray gray gray\">Subcon:  {$con_name}</td></tr>";
        $info_html.="<tr><td colspan=\"2\" width='50%' style=\"border-width: 1px;border-color:gray gray gray gray\">{$signature_html_1}</td><td colspan=\"2\" width='50%' style=\"border-width: 1px;border-color:gray gray gray gray\">Verified By:</td></tr>";
        $info_html.="</table>";
...
//            $sql = "select count(apply_id) as cnt,program_id,status from ptw_apply_basic where record_time like '".$month."%' and program_id ='".$args['program_id']."'  GROUP BY status";
    ...
    public static function AllCntList3($args){
        ...
        if($pro_model->main_conid != $args['contractor_id']){
            //SUBSTRING_INDEX(a.add_operator, '|', 1)
            $root_proid = $pro_model->root_proid;
            $sql = "select count(check_id) as cnt,project_id,status from rf_record where project_id = '".$root_proid."' and contractor_id = '".$args['contractor_id']."' GROUP BY status";
//            $sql = "select count(apply_id) as cnt,program_id,status from ptw_apply_basic where program_id = '".$args['program_id']."' and record_time like '".$month."%' and apply_contractor_id = '".$args['contractor_id']."'  GROUP BY status";
        }else{
//            $sql = "select count(apply_id) as cnt,program_id,status from ptw_apply_basic where record_time like '".$month."%' and program_id ='".$args['program_id']."'  GROUP BY status";
